...
- 커밋 본문에는 무엇을 왜 변경했는지와 주요 검증 결과를 작성하고, 문장은 `~했음.`, `~함.` 형식으로 끝낸다.
- 이전 대화에서 커밋을 허용했더라도 새로운 변경을 commit 또는 push하기 전에는 매번 사용자에게 별도 허락을 받는다.
- 사용자의 명시적인 현재 승인 없이 commit, push, pull request를 수행하지 않는다.
