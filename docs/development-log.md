...
### 영상 연결·취소·종료·재연결의 비동기 경계 보완

- 이전 History 작업은 원인·수정 동작·검증·남은 범위를 상세히 적은 `9b79f79`로 커밋하고 사용자 승인으로 `origin/main`에 푸시했다. 이번 영상 변경은 별도 미커밋 상태로 유지한다.
- 기존 WhepClient는 ICE·HTTP를 무기한 기다렸고 DELETE가 끝나야 로컬 PeerConnection을 닫았다. 연결 중 중지 버튼도 비활성화돼 있었다. DELETE가 응답하지 않으면 로컬 종료가 지연되는 회귀 테스트 실패를 수정 전에 확인했다.
- 취소 가능한 10초 제한 작업을 영상 기능 내부에 추가했다. 시그널링 요청과 WHEP 연결 준비 전체에 각각 적용하고, 로컬 종료는 트랙·PeerConnection·store·영상 요소부터 즉시 정리하도록 했다. WHEP DELETE와 백엔드 stop은 각 10초 안에서 별도로 시도하므로 다음 연결을 막지 않는다.
- 연결 시도마다 독립적인 WHEP 객체·취소 컨트롤러·세션 ID를 관리한다. 이전 응답·트랙·상태 이벤트는 새 시도에 반영하지 않으며, 늦게 받은 Location이나 백엔드 세션 ID는 가능한 경우 정리한다. 로봇·인증 세션 전환 시 기존 클라이언트를 닫고 새 클라이언트와 로봇별 영상 요소를 사용한다.
- SDP 교환이나 ontrack만으로 연결 완료를 표시하지 않고 실제 PeerConnection connected를 기다린다. 영상 요소의 프레임 콜백(미지원 시 재생 시간 진행)을 별도로 관측해 수신 대기·수신 중·중단을 표시한다. 1초 간격 검사에서 3초 이상 새 프레임이 없으면 수신 중단으로 바꾸고 수신 시 복구한다. 마지막 프레임 시각을 표시하고 샘플에는 실제 프레임 수신을 표시하지 않는다.
- 가짜 fetch·PeerConnection·시그널링·타이머·영상 프레임 콜백을 이용한 관련 6파일 37개 테스트가 통과했다. ICE·HTTP·본문·offer·실제 연결 대기 시간 초과, ICE 중 취소와 리스너 제거, 늦은 Location 삭제, answer 실패 정리, A→B 전환, 재연결 실패, 프레임 중단·복구, unmount 정리를 확인했다. 초기 검증의 테스트 matcher가 null을 Object로 처리한 오류는 기대 호출을 명확히 바꾼 뒤 통과했다.
- 운영 빌드·TypeScript·ESLint·diff 검사가 통과했다. 백엔드·의존성·Jetson 송출기·MediaMTX 설정은 변경하지 않았고 백엔드 및 전체 테스트를 반복하지 않았다. 실제 서비스·장비·영상 연결은 검증하지 않았다.
- 원격 삭제 응답 유실이나 세션 ID/Location 미수신 시 서버 세션 정리 완료는 보장하지 않는다. 로그아웃 후 이전 백엔드 세션 정리를 새 인증 정보로 수행하지 않는다. Jetson의 상시 송출과 자동 품질 조정 미구현은 기존 제한으로 유지한다.

## 2026-09-10

### History 실제 조회와 UTC 날짜·미집계 표시

- 이전 STOMP 권한 변경을 사용자 승인으로 `ff3c8c5`에 커밋한 뒤 History만 수정했다. 명시적 개발 Mock과 실제 로봇 store 사용은 이미 적용돼 있어 유지하고 관련 회귀 테스트로 확인했다.
- 날짜 문자열을 그대로 보내던 문제를 UTC 시작일 00:00:00부터 종료일 23:59:59.999999999까지의 ISO Instant 범위로 수정했다. 날짜 누락·유효하지 않은 날짜·역전 범위를 조회 전에 거부하고 offset 없는 서버 응답 시각은 UTC로 해석한다.
- 검색 중·실패·빈 결과·검색 전 상태와 실제·샘플을 구분했다. 새 검색·필터·세션 변경 후 늦은 성공·실패 응답을 무시하고 오래된 결과는 표시하지 않는다.
- 백엔드 미집계 거리·커버리지의 가짜 0을 null로 바꾸고 화면에 미집계를 표시한다. 좌표 한 개는 Point와 마커로 표시하며 두 개 이상은 기존 LineString을 유지한다. 거리 계산·커버리지 알고리즘·작업 세션 및 이벤트 집계는 구현하지 않았다.
- 수정 전 프론트 API 테스트 실패 4개, 백엔드 서비스 테스트 실패 2개로 날짜 및 응답 문제를 재현했다. 수정 후 관련 프론트 5파일 26개, 백엔드 서비스·컨트롤러 8개 테스트가 통과했다. 테스트의 로그아웃 함수명 오타를 수정한 뒤 해당 9개 테스트와 빌드만 재실행했다. 컨트롤러에는 UTC 종료일 나노초 경계 전달 검증을 보강해 해당 5개 테스트를 재실행했다.
- 프론트 운영 빌드·TypeScript·ESLint, 백엔드 Java 컴파일·bootJar 및 diff 검사가 통과했다. 실제 서버·DB·MQTT·장비·지도 API에는 연결하지 않았다. 백엔드 별도 린트 작업과 전체 테스트 실행은 없었으며 이번 History 변경은 미커밋 상태다.
...
- STOMP events의 필수 필드·심각도·시각·topic 로봇 ID를 검증하고 기존 Zustand 기반 recentEventsStore에 합친다. 동일 ID는 중복 표시하지 않고 시각순 최신 3건을 유지한다. 조회 중 먼저 받은 실시간 이벤트를 늦은 초기 응답이 덮어쓰지 않도록 했다.
- 로봇 전환·재선택·로그아웃 뒤 도착한 이전 요청의 성공과 실패를 무시한다. 최근 이벤트는 로그아웃 시 비우고 조회 중·조회 오류·빈 결과·샘플 모드를 구분한다. 조회 실패 이후에도 수신한 실시간 이벤트는 표시한다.
- 로그 API의 UTC LocalDateTime과 STOMP offset 시각을 최근 목록 안에서만 같은 기준으로 정렬했다. 로그 API 설계, 텔레메트리 감지 코드와 의존성은 이번 변경에서 다시 수정하지 않았다.
- 수정 전 실제 API 이벤트가 표시되지 않는 회귀 테스트 실패 1건을 확인했다. 수정 후 관련 프론트 7파일 30개 테스트, TypeScript·운영 빌드·ESLint·diff 검사가 통과했다. 이미 검증한 텔레메트리와 백엔드 전체 테스트는 반복하지 않았다.
- 실제 서비스·장비·MQTT·DB·브라우저 실연동은 실행하지 않았다. 기존 로그 API에는 별도 최근 N건 제한이 없어 초기 응답을 받은 뒤 3건을 추린다. 이전 Mock 분리 작업만 e4ebc20으로 커밋했으며 텔레메트리 감지와 이번 이벤트 연결은 아직 커밋하지 않았다.

### 텔레메트리 수신 지연 감지와 상태 전이 표시

- 서버가 Edge `receivedAt`을 마지막 수신처럼 전달하던 문제를 수정해, 처리 진입 시 `Clock`으로 잡은 서버 시각을 DB·STOMP에 사용하고 Edge 시각을 별도 필드로 보존했다.
- 로봇별 서버 수신 상태를 never-seen·normal·delayed로 관리한다. 250ms 주기 검사에서 3초 이상 중단을 감지하고 새 수신으로 복구한다. 로봇 조회·기존 status 응답에 상태 스냅샷을 추가했다. 지연·복구와 Edge 상태 변화만 DB 이벤트로 기록한다.
- 브라우저는 서버에서 지난 시간과 로컬 경과 시간을 합산해 메시지가 없어도 상태 패널을 갱신한다. 이 감지에는 물리 정지·MQTT 명령 발행을 추가하지 않았다.
- 수정 전 최소 회귀 테스트 두 건으로 화면 갱신 중단과 동일 status 중복 저장을 재현했다. 수정 후 가짜 시계의 경계·중단·복구·로봇별 독립성·시계 차이·중복 억제를 포함한 백엔드 관련 테스트 23개와 프론트 관련 테스트 37개가 통과했다. 백엔드 bootJar, 프론트 운영 빌드·TypeScript·ESLint도 통과했다. 전체 테스트는 반복하지 않았다.
- 실제 DB·MQTT·장비·브라우저 실연동은 실행하지 않았다. 서버 수신 상태는 프로세스 메모리 기준이어서 재시작 후 never-seen에서 다시 시작한다. 이 변경은 아직 커밋하지 않았다.

### 실제 모드의 샘플 데이터와 이전 세션 데이터 혼입 차단

- 시작 시 Git 작업 트리는 깨끗했으며 AGENTS.md, SRS의 F-02·F-03·F-12·N-16과 관련 책임 문서·호출 경로를 확인했다. 로봇 조회 백엔드는 저장소 결과를 반환하므로 API·MQTT 계약이나 의존성을 바꾸지 않고 프론트 경계를 수정했다.
- 모든 Mock을 개발 환경의 명시적 `true`로 제한했다. 운영 빌드에서는 Mock 관리자 세션을 생성하지 않고 저장된 Mock 토큰도 복원하지 않는다. 로봇·텔레메트리는 빈 상태로 시작하고, 조회 실패는 오류로 표시하며 사라진 선택은 유효한 로봇 또는 선택 없음으로 전환한다.
- 샘플 지도·텔레메트리는 연결 문자열 대신 명시적인 데이터 출처로 구분한다. 실제 GPS 미수신이나 지도 오류를 샘플 위치·경로로 대체하지 않는다. 기존 개발 Mock은 유지하고 `.env.example`은 실제 모드를 기본으로 안내한다. 기존 `.env.local`의 명시적 Mock 설정은 보존했다.
- 로그아웃은 장비·제어·구역·영상 상태와 조회·변경 캐시를 제거한다. 세션 번호를 사용해 늦은 HTTP·STOMP·제어·영상 응답이 데이터를 다시 채우지 못하게 했다. History는 초기 샘플·설정·로봇 선택 경계만 수정하고 영상은 세션 경계만 보강했으며 내부 흐름은 재작성하지 않았다.
- 수정 전 회귀 테스트 실패 6건으로 기본 Mock 활성화·운영 Mock·샘플 초기값·선택 보정 누락을 재현했다. 수정 후 전체 프론트 테스트 40파일 126개가 통과했다. 마지막 History 선택 보정 후 관련 2파일 3개를 추가 실행해 통과했고 TypeScript·운영 빌드와 ESLint도 통과했다.
- 실제 장비·운영 서비스·실제 DB·브라우저 실연동은 검증하지 않았다. 백엔드 코드는 변경하지 않아 백엔드 빌드·테스트는 실행하지 않았다. History API 내부 흐름, 영상 연결 수명주기 전반과 텔레메트리 노후화 개선은 후속 범위로 남긴다. commit·push·PR은 수행하지 않았다.


### 로그 날짜 조건을 로봇 선택과 독립적으로 적용

- 기존 미커밋 검색어 연동과 스냅샷 응답을 보존하고, 로봇 미선택 시 날짜 조건이 빠지던 서비스 분기를 수정했다. 날짜 범위는 DB 쿼리에서 적용하며 조회 결과를 메모리에서 날짜 필터링하지 않는다.
- 시작·종료 시각은 각각 독립적인 포함 경계로 처리하고, 역전 범위는 기존 오류 응답 구조의 `400 INVALID_REQUEST`로 거부한다. 프론트는 비운 날짜를 생략하고 검색 실패 메시지를 표시하며 기존 조회 결과를 유지한다. UTC 날짜 해석을 유지하고 Mock 검색에도 출처와 한쪽 날짜 조건을 적용했다.
- 수정 전 백엔드 회귀 테스트 2개와 프론트 회귀 테스트 4개의 실패로 문제를 재현했다. 수정 후 백엔드 로그·스냅샷·관련 컨트롤러 테스트 39개, 프론트 로그 API·화면·관련 컴포넌트 테스트 8개가 통과했다. 프론트 린트와 production build도 통과했다.
- 백엔드 테스트는 저장소를 모의 처리하므로 실제 PostgreSQL 쿼리 실행과 실행 계획은 검증하지 않았다. 실제 장비·운영 서비스·MQTT 명령은 실행하지 않았다. 페이지네이션과 DB 조회 후의 기존 검색어 필터 최적화는 이번 범위에서 제외했다.

## 2026-09-09

### 작업 지침과 과거 계획의 적용 범위 정리

- 로컬 `AGENTS.md`에서 관련 문서의 확인 범위, 요구사항·구현·실행 증거의 구분, 질문이 필요한 조건과 변경 영향에 맞는 검증 기준을 명확히 했다. 특정 모델이나 추론 수준에 종속되는 규칙은 추가하지 않았다.
- 로컬 개발 로드맵에 기존 웹·백엔드 수정 우선순위와 하드웨어 담당자 인계 범위를 추가하고, 6월 계획과 검증 결과는 당시 기록으로 보존했다.
- 두 과거 계획 문서의 Superpowers 필수 실행 지시를 역사적 참고로 바꾸고, 체크리스트가 현재 작업 실행이나 commit·push 승인이 아님을 명시했다.
- 문서 변경의 공백 오류, 과거 본문 보존, 기존 미커밋 코드와 `SRS.md`의 변경 여부를 확인했다. 애플리케이션 테스트와 실제 하드웨어 검증은 문서 수정 범위에 해당하지 않아 실행하지 않았다.
- `AGENTS.md`와 로드맵의 기존 Git 제외 설정을 유지했다. 이번 작업에서는 commit·push를 수행하지 않았다.

## 2026-06-16

### 로그 뷰어 실제 검색어 필터 연동

- 백엔드 `/api/logs`가 `text` 파라미터를 받아 로그 message, eventType, source를 대소문자 구분 없이 필터링하도록 했다.
- 프론트 로그 뷰어는 실제 API 모드에서 검색어가 비어 있지 않을 때만 `text` 파라미터를 전송하도록 정리했다.
- 로그 화면의 검색어 입력 안내 문구를 한국어로 정리해 사용자가 메시지, 이벤트 유형 또는 출처를 검색할 수 있음을 표시했다.
- 백엔드 서비스와 컨트롤러 테스트, 프론트 로그 API 테스트를 추가해 검색어 전달과 필터링을 검증했다.
- 백엔드 전체 테스트, 프론트 테스트 97개, 린트와 production build 통과를 확인했다.

## 2026-06-15

### 수동 카메라 스냅샷 저장과 로그 조회

- WebRTC `<video>`의 현재 프레임을 Canvas에서 JPEG로 캡처해 백엔드 multipart API로 업로드하도록 구현했다.
- 스냅샷 저장 중 중복 요청을 막고, 프레임 미수신 또는 업로드 실패 원인을 카메라 패널에 표시하도록 했다.
- `robot_snapshot` 테이블과 `robot_event.snapshot_id` 연결을 추가하고 JPEG 파일은 설정된 로컬 디렉터리에 저장하도록 했다.
- JPEG 형식과 5MB 제한을 검증하고, DB 저장 실패 시 먼저 저장한 파일을 정리하도록 했다.
- 수동 저장 시 `manual-snapshot`, `dashboard`, `info` 로그 이벤트를 생성하도록 했다.
- 로그 응답에 스냅샷 참조를 포함하고, 로그 뷰어가 인증된 JPEG 응답을 Blob으로 받아 미리보기하도록 구현했다.
- 백엔드 저장소·서비스·권한 테스트와 프론트 캡처·HTTP·상태 테스트를 추가했다.
- 백엔드 테스트 70개, 프론트 테스트 95개, 린트와 production build 통과를 확인했다.
- 로컬 브라우저에서 스트림 연결 전 스냅샷 버튼 비활성화를 확인했다.
- 원격 MediaMTX `100.92.7.56:8889` 연결이 시간 초과되어 실제 영상 프레임 캡처와 저장 후 로그 미리보기 통합 검증은 남아 있다.

### 네이버 위성 지도 전환

- 국내 현장 위성영상 품질을 높이기 위해 MapLibre와 MapTiler 배경을 네이버 Dynamic Map으로 교체했다.
- 지도 페이지의 작업 구역, 완료·예정 경로, 로봇 위치를 각각 Polygon, Polyline, Marker 오버레이로 연결했다.
- 작업 구역 편집 시 지도 클릭으로 꼭짓점을 추가하고 Marker 드래그로 좌표를 이동하도록 구현했다.
- History 화면의 과거 경로와 이벤트 위치도 같은 네이버 위성 지도에 표시하도록 통일했다.
- `VITE_NAVER_MAP_CLIENT_ID` 기반 SDK 로더를 추가하고 Client ID 누락 또는 SDK 실패 시 기존 fallback 지도를 유지했다.
- MapLibre 패키지와 MapTiler 전용 스타일 코드를 제거했다.
- 프론트 테스트 91개와 production build 통과를 확인했다.
- MapLibre 기본 CSS 제거 후 네이버 지도 컨테이너 높이가 0이 되는 문제를 확인해, 지도 컴포넌트가 부모 영역의 너비와 높이를 직접 채우도록 수정했다.
- 브라우저에서 지도 높이 약 242px, 네이버 위성 타일 28개 로드, fallback과 콘솔 오류 없음으로 실제 표시를 검증했다.

### 지도 기본 위치 변경

- `MOWER-01` 지도 기준 위치를 위도 `36.6259428230794`, 경도 `127.45455324663685`로 변경했다.
- 지도 초기 중심, 샘플 텔레메트리, 샘플 경로 끝점과 fallback 지도 범위가 같은 기준 좌표를 사용하도록 통일했다.
- 기존 샘플 작업 구역과 경로는 형상을 유지하면서 새 위치로 이동했다.
- 실제 GPS 수신 좌표와 PostGIS에 저장된 작업 구역은 변경하지 않았다.
- 지도 좌표 일관성 회귀 테스트를 추가했고 프론트 테스트 86개 통과를 확인했다.

### 위성 지도 화질 비교 설정

- MapTiler Satellite가 실제로 제공하는 512px raster tile 크기에 맞춰 `tileSize`를 512로 수정했다.
- 지도 초기 줌을 18로 높이고 최대 운용 줌을 19로 제한했다.
- 원본 영상에 없는 세부 정보를 과도하게 확대하는 것을 막으면서 작업 구역을 비교할 수 있도록 조정했다.
- 지도 스타일과 MapLibre 초기화 옵션 회귀 테스트를 추가했고 프론트 테스트 87개 통과를 확인했다.

### MapTiler 위성 지도 연결

- 기존 MapLibre demo style 대신 MapTiler Satellite TileJSON을 사용하는 raster style 생성 함수를 추가했다.
- `VITE_MAPTILER_API_KEY`가 있을 때만 외부 위성 지도를 초기화하도록 구성했다.
- API 키가 없거나 MapLibre 초기화·위성 tile 로딩이 실패하면 기존 관제용 fallback 지도를 유지하도록 했다.
- 실제 API 키는 `.env.local`에서 관리하고 `.env.example`에는 placeholder만 제공하도록 정리했다.
- 지도 스타일과 fallback 회귀 테스트를 추가했고 프론트 테스트 83개 통과를 확인했다.

### 만료된 프론트 인증 세션 정리

- 헤더에 `admin`이 표시되더라도 저장된 JWT가 만료되면 보호 API가 401을 반환할 수 있음을 확인했다.
- 인증이 필요한 HTTP 요청에서 401을 받으면 `sessionStorage`와 Zustand 인증 상태를 함께 해제하도록 수정했다.
- `/map`, `/history`, `/logs`, `/settings`를 `RequireAuth` 아래에 배치해 미인증 접근 시 `/login`으로 이동하도록 했다.
- 로그인 요청 자체의 401은 `skipAuth` 요청이므로 기존 세션 정리 처리에서 제외했다.
- 인증 회귀 테스트를 추가했고 프론트 테스트 79개 통과를 확인했다.

### 작업 구역 실제 저장·조회 연동

- `VITE_ENABLE_MOCK_WORK_ZONE` 환경변수로 샘플 모드와 실제 백엔드 연동 모드를 분리했다.
- 백엔드 GET 응답의 Polygon, zone ID, version, 갱신 시각 계약을 프론트 타입에 반영했다.
- 작업 구역 저장 시 현재 version을 `expectedVersion`으로 전달하고 성공 응답의 새 version을 저장하도록 구현했다.
- 404 조회는 등록된 구역 없음으로 처리하고, 조회·저장 실패 시 편집 내용을 보존하면서 한국어 오류 메시지를 표시하도록 했다.
- 실제 저장 모드에서는 샘플 구역 불러오기 액션을 숨겼다.
- 프론트 테스트 74개, 린트와 production build 통과를 확인했다.

### 작업 구역 저장 통합 검증과 수정 UX

- 실제 Spring Boot API를 통해 빈 PostGIS 작업 구역을 최초 저장하고 version 1을 확인했다.
- GET 재조회에서 Polygon, SRID 4326과 꼭짓점 5개가 복원되는 것을 확인했다.
- 좌표 수정 저장 후 version이 2로 증가했고, 오래된 version 1 요청은 HTTP 400으로 거부됐다.
- PostGIS `ST_IsValid` 결과가 true인 것을 확인했다.
- 저장된 Polygon을 `기존 구역 수정`으로 불러오고 `새 구역 다시 그리기`와 분리했다.
- MapLibre와 fallback 지도에서 꼭짓점 드래그 이동을 지원하도록 구현했다.
- `편집 취소` 시 저장된 Polygon으로 돌아가도록 했다.
- 프론트 테스트 77개, 린트와 production build 통과를 확인했다.
- 네이버 지도 전환 후 실제 로그인 세션에서 기존 작업 구역의 첫 꼭짓점을 드래그해 이동했다.
- 저장 전 첫 좌표 `127.454324, 36.626050`이 저장 후 `127.454249, 36.626102`로 변경되는 것을 화면에서 확인했다.
- PostGIS 작업 구역 version이 6에서 7로 증가했고 SRID 4326과 `ST_IsValid=true`를 확인했다.
- 브라우저를 새로고침한 뒤 같은 네 좌표가 다시 표시되어 저장·재조회 흐름을 확인했다.

### GPS 연결 전 지도 경로 시각화

- GPS가 없는 시연 환경에서도 지도 기능을 확인할 수 있도록 샘플 경로를 완료 구간과 예정 구간으로 분리했다.
- Mock 연결에서는 좌표가 유효해도 실제 GPS로 표시하지 않고 `샘플 운용 데이터`, `샘플 위치`로 명확히 구분했다.
- 실제 STOMP 연결에서 유효한 GPS 좌표가 수신되면 로봇별 세션 경로를 최대 500개 좌표까지 누적하도록 구현했다.
- 연속된 두 좌표를 기준으로 북쪽 기준 진행 방향을 계산하고 marker 화살표에 반영했다.
- 지도 tile 초기화 실패 화면에서도 샘플 작업 구역, 완료·예정 경로와 방향 marker가 유지되도록 정리했다.
- 실제 거리, 작업 면적과 커버리지 값은 계산하거나 실제 값처럼 표시하지 않았다.
- 지도 클릭으로 작업 구역 꼭짓점을 추가하고 3개 이상이면 Polygon 미리보기를 표시하도록 구현했다.
- MapLibre 지도와 fallback 지도 모두 작업 구역 편집을 지원하도록 편집 상태를 공유했다.
- 마지막 점 취소, 전체 초기화, 샘플 구역 불러오기와 저장 요청 기능을 추가했다.
- 개발 모드 저장은 실제 DB 저장이 아님을 사용자에게 표시하도록 했다.
- 프론트 테스트 69개, 린트와 production build 통과를 확인했다.

## 2026-06-14

### 시연용 영상 스택 통합 실행

- RealSense 카메라, MediaMTX와 NVENC 영상 송출기를 한 명령으로 실행하는 `run-video-demo.sh`를 추가했다.
- 컬러 원본 ROS 2 토픽과 MediaMTX API의 준비 상태를 확인한 뒤 다음 구성요소를 시작하도록 구성했다.
- 구성요소 하나가 종료되거나 사용자가 종료 신호를 보내면 함께 시작한 프로세스 그룹을 모두 정리한다.
- Bash 문법 검사, Jetson 영상 테스트 7개와 카메라 스크립트 테스트 5개 통과를 확인했다.
- 실제 Jetson 장비에서 통합 실행과 브라우저 `Start Stream`, `Stop Stream`, `Reconnect` 반복 검증은 후속 시연 점검으로 남아 있다.
...
## 2026-09-23 — 받은 CPP 소스의 오프라인 반입

- 받은 `mower_gps/mower_web/mower_ws/src/mower_map`의 소스·테스트·패키지 설정을 `edge/mower-navigation/src/mower_map`에 반입했다. 기존 웹·백엔드 미커밋 변경은 보존했다.
- Python 실행 스크립트로 ROS 없이 임시 폴더에서 C++17 빌드와 테스트를 실행하도록 했다. 원본 테스트가 `std::invalid_argument`를 사용하면서 `<stdexcept>`를 직접 포함하지 않아 MinGW에서 실패하는 문제를 수정했다.
- Windows Python 3.13 및 MinGW g++에서 grid_map_test, coverage_planner_test, CLI smoke test를 통과했다. CLI 검증은 2706개 경로점과 위경도 정밀도를 확인했다. 원본의 미사용 approxEqual 함수 경고는 남아 있다.
- CMake/colcon, Jetson ROS, GPS, 웹 연결, STM32 및 실기체는 실행하지 않았다. 경계 이탈 방지·장애물 우회·정지 중재의 미완성 조건을 README와 로드맵에 명시했다.


## 2026-09-23 — 웹 CPP 예정 경로 미리보기

저장된 작업 구역에서 사용자가 명시적으로 요청한 경우에만 기존 C++ CLI를 실행하고 지도에 예정 경로를 표시하도록 연결했다. SRS I-1·I-3 및 F-30의 저장 구역을 활용하는 검토 기능이며 I-7 커버리지 90%나 실기체 주행 안전 달성을 주장하지 않는다.

### 변경 범위와 기존 작업 보존

작업 시작 당시 제어권·ACK·조이스틱, 작업 구역 버전/동시성, 지도 편집 제한, UI 정리, 관련 문서와 `edge/mower-navigation` 반입이 미커밋 상태였다. 이를 보존했다. 기존 `WorkZoneService`·엔티티·저장소·V8 마이그레이션과 제어/MQTT 코드는 이번에 수정하지 않았다.

이번 추가는 `CppPreviewController`, 요청/응답 DTO, `CppPreviewService`, `CppProcessRunner`와 테스트 4개 파일, `ErrorCode`의 CPP 오류, `application.yml`의 CPP 설정이다. 프런트는 `cppPreview.ts`·테스트·`CppPreviewControls.tsx`를 추가하고 기존 두 지도 컴포넌트·지도 테스트·스타일에 표시를 연결했다. 반입한 C++ 알고리즘 소스는 변경하지 않았고 검증 스크립트에 성공한 CLI를 보관하는 `--output`만 추가했다.

서버는 GeoJSON `[경도, 위도]`를 CPP `{lat, lon}`으로 바꾸고 저장 버전을 실행 전후 확인한다. 입력 꼭짓점/격자, 좌표 범위, 실행 시간, stdout 크기와 동시 실행을 제한한다. 실행 파일은 서버가 지정한 절대 경로로 셸 없이 호출한다. 웹은 요청 세대 번호와 취소 신호를 사용해 로봇·구역·편집·세션 변경 후 늦게 도착한 응답을 버린다. 전체 경로선과 시작·끝·순서 번호·방향 화살표를 표시하며 패널을 접어 경로를 볼 수 있다.

### 이번 실행 결과

- `verify_offline.py`: C++ 테스트 2개 및 CLI 정밀도 테스트 통과. 정밀도 테스트 경로점 2,706개. 기존 C++ 테스트의 사용하지 않는 함수 경고 1개는 유지했다. `--output`으로 서버용 CLI를 보관한 재실행도 통과했다.
- Java 21 Gradle 관련 테스트 35개 통과, skip 0. 실제 CLI를 실행하는 `CppNativeIntegrationTest` 1개 포함이며 저장 구역 조회는 Mock이다. 별도 자식 Java 프로세스로 실패·입력 대기 중 시간 초과·출력 초과·프로세스 종료·동시 실행 제한을 검증했다. API 인증·권한·입력·시간 초과 응답을 검사했고 `bootJar`가 통과했다.
- 프런트 첫 전체 실행: 47개 파일 211개 테스트 통과. npm 래퍼가 필터를 전달하지 않아 전체가 실행된 것으로 확인했다. 직접 CLI로 지도 관련 11개 파일 53개도 통과했다.
- 이후 대체 지도 CPP 표시 회귀 테스트와 패널 접기를 추가하고 최종 영향 범위 3개 파일 26개 테스트를 다시 실행해 통과했다. 최종 TypeScript 포함 운영 빌드와 수정 지도 파일 ESLint 통과. 전체 테스트를 최종 변경 후 다시 실행한 것은 아니다.
- 로컬 브라우저: 실제 C++가 만든 130점 결과를 임시 테스트 대역 구역·HTTP 응답에 연결해 생성 중, 시작·끝·번호·방향, 패널 접기와 구역 변경 후 결과 제거를 확인했다. 임시 화면 파일과 개발 서버·브라우저 탭은 정리했다.
- `git diff --check` 통과. `.local-docs/failure-learning.md`에는 테스트 필터 래퍼의 동작을 기록했으며 Git 제외 상태를 확인했다.

### 남은 제약

실제 PostgreSQL 저장→인증된 HTTP→네이버 실 SDK 전체 연결과 실제 네이버 배경 지도는 이번에 검증하지 않았다. 결과는 저장하지 않으며 다른 사용자의 구역 저장은 재조회나 다음 생성 시 확인한다. 내부 ring은 지원하지 않는다. 경로점 사이 연결의 경계 통과, 장애물 우회, 기체 폭·회전 반경, 실기체 커버리지는 검증하지 않는다. Jetson SSH·배포·ROS/MQTT 주행·STM32·칼날·모터 작업은 수행하지 않았다. commit·push·PR도 수행하지 않았다.

실행 방법과 제한은 [작업 구역 흐름](learning/07-work-zone-postgis.md#cpp-예정-경로-미리보기) 및 [CPP README](../edge/mower-navigation/README.md#웹-미리보기용-실행-파일), 계약은 [API 문서](api-contract.md)를 따른다.


## 2026-09-24 — CPP 미리보기 리뷰 보완 3건

2026-09-23에 전달받은 리뷰의 무기한 요청 대기, 위치 갱신에 의한 지도 자동 이동, 작은 구역의 큰 격자 오류 분류를 현재 소스에서 확인하고 수정했다. 기존 제어·ACK·구역 동시성 변경과 C++ 알고리즘은 수정하지 않았다.

- `cppPreview.ts`: HTTP 응답 본문 처리까지 15초 제한을 적용했다. 시간 초과 시 생성 중 상태를 끝내고 요청을 취소하며 재시도를 안내한다. 취소를 무시하는 늦은 응답도 결과를 덮어쓰지 못하도록 Promise 경쟁과 기존 요청 세대 검사를 함께 사용한다. 성공·구역 변경·컴포넌트 해제 시 타이머를 정리한다. 브라우저의 취소는 서버 DB 처리 취소를 보장하지 않는다.
- `NaverOperationalMap.tsx`: 예정 경로가 표시되는 동안에는 로봇 위치 마커만 갱신하고 panTo를 생략한다. 예정 경로가 없어지면 기존 자동 추적을 재개한다. SDK 대역 테스트로 경로 표시 직후와 위치 갱신 시 화면 이동 억제, 위치 마커 갱신, 추적 재개를 확인했다.
- `CppPreviewService`·`ErrorCode`: C++의 평균 위도 보정에 맞춰 구역의 남북 높이와 동서 폭을 검사한다. 한 칸도 만들 수 없으면 CLI를 호출하지 않고 422 `CPP_GRID_TOO_LARGE`와 격자 간격 축소 안내를 반환한다. 프런트도 해당 오류를 한국어로 안내한다. 격자는 생성 가능하지만 경로가 없는 경우는 기존 200 빈 결과 계약을 유지한다.

이번 실행 결과:

- C++ 오프라인 검증 3종 통과. 기존 사용하지 않는 테스트 함수 경고 1개는 남아 있다.
- 백엔드 CPP 테스트 20개 통과, skip 0. Gradle `--rerun-tasks`로 실행했으며 실제 CLI 테스트 1개를 포함한다. 구역 조회는 Mock이다. `bootJar`도 재생성했다.
- 프런트 관련 3개 파일 30개 테스트 통과. 무응답 시간 초과·재시도·늦은 응답·타이머 정리·자동 추적 억제/재개·422 안내를 포함한다.
- TypeScript 포함 운영 빌드, 수정 프런트 파일 ESLint와 `git diff --check` 통과.
- 이전 전체 211개 테스트와 브라우저 화면 검증은 이번 보완에서 반복하지 않았다. 실제 DB→HTTP→네이버 실 SDK 전체 연결과 실기체 주행은 미검증이다.

API 계약과 작업 구역 책임 문서에 15초 제한, 자동 추적 억제 및 422 계약을 반영했다. 로컬 실패 학습 기록도 갱신했으며 commit·push·PR은 수행하지 않았다.


## 2026-09-24 — CPP 커밋 범위 분리 검증

CPP 구현·보완, 반입한 C++ 모듈 및 지도 준비 상태·저장 구역 좌표 도식의 표시 의존 부분을 커밋 대상으로 분리했다. 제어·ACK·조이스틱·작업 구역 DB 동시성·미사용 코드 정리는 미커밋으로 보존했다. 공유 문서와 스타일·오류 코드는 관련 부분만 스테이징했다.

스테이징한 트리를 별도 로컬 디렉터리에 추출하여 프런트 CPP 관련 30개 테스트와 운영 빌드, 백엔드 CPP 20개 테스트(skip 0) 및 bootJar를 재검증했다. 백엔드 실제 CLI 테스트의 구역 조회는 Mock이다. 커밋 대상의 비밀키·토큰 패턴 검사와 diff 검사를 통과했다. 푸시는 하지 않는다.
